    ...
    return loadConfig(path.join(basePath, TRANSIFEXRC)).then(function(rc) {
        if((!project && Object.keys(rc).length > 0) || project in rc) {
            ...
            return loadConfig(os.homedir(), TRANSIFEXRC).catch(function() {
                throw "Couldn't load the transifexrc from the home directory. Loading was attempted, since the project's transifex RC is either empty or does not contain the requested project.";
            });
        }
    });
};
