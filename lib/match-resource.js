...
function matchFileFilter(basePath, localPath, fileFilter) {
    const rmatch = localPath.match(new RegExp("^" + path.join(basePath, fileFilter.replace(/<lang>/g, "([^\\/]+)")) + "$"));
...
module.exports = function(basePath, localPath, resource) {
    if("source_file" in resource && localPath == path.join(basePath, resource.source_file)) {
        ...
        if(fileFilterResult) {
            ...
            const explicitPaths = Object.keys(resource).filter((key) => key.search(/trans\..+/) != -1);
            for(const k in explicitPaths) {
                const key = explicitPaths[k];
